...
import { text, isCancel, cancel, intro, outro, log } from "@clack/prompts";
...
async function getAIResponse(conversationId) {
  ...
  try {
    ...
    spinner.error("failed to get an ai answer");
    throw error;
  }
}
...
async function chatLoop(conversation) {
  ...
  while (true) {
    const userInput = await text({
      message: chalk.blue("Your mesage"),
...
export async function startChat(mode = "chat", conversationId = null) {
  try {
    intro(boxen(chalk.bold.cyan("Orbital AI Chat")), {
      padding: 1,
      borderStyle: "double",
      borderColor: "cyan",
    });
